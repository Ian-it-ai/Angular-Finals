...
export class PostService {
  ...
  deleteButton(index: number) {
    ...
    this.http.delete(`https://final-project-92107-default-rtdb.asia-southeast1.firebasedatabase.app/posts/${index}.json`).subscribe(() => {
      console.log('Post deleted from Firebase');
    });
  }
  addPost(post: Post) {
    this.listOfPosts.push(post);
    this.http.put(`https://final-project-92107-default-rtdb.asia-southeast1.firebasedatabase.app/posts.json`, this.listOfPosts)
      .subscribe(response => {
  ...
  likePost(index: number) {
    ...
    this.http.put(`https://final-project-92107-default-rtdb.asia-southeast1.firebasedatabase.app/posts/${index}.json`, this.listOfPosts[index])
      .subscribe(() => {
  ...
  editComment(postIndex: number, commentIndex: number, newComment: string) {
    ...
    // Update the post in Firebase
    this.http.put(`https://final-project-92107-default-rtdb.asia-southeast1.firebasedatabase.app/posts/${postIndex}.json`, this.listOfPosts[postIndex])
      .subscribe(() => {
